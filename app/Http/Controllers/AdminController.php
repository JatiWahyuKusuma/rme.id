...
class AdminController extends Controller
{
    ...
    {
        $admins = AdminModel::select('no', 'level_id', 'nama', 'email','opco', 'password');
        if ($request->nama) {
        ...
        return Datatables::of($admins)
            ...
            ->addColumn('aksi', function ($admin) {
                $btn  = '<a href="' . url('/admin/' . $admin->no) . '" class="btn btn-info btn-sm">Detail</a> ';
                $btn .= '<a href="' . url('/admin/' . $admin->no . '/edit') . '" class="btn btn-warning btn-sm">Edit</a> ';
                $btn .= '<form class="d-inline-block" method="POST" action="' . url('/admin/' . $admin->no) . '">'
